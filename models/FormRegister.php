...
class FormRegister extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            //['UserName','match','pattern' => "/^([a-zA-ZÃ±-Ã_\u00f1\u00d1\u00E0-\u00FC])\w+/",'message'=>'Sólo se aceptan letras'],
            //['UserLastName','match','pattern' => "/^([a-zA-ZÃ±-Ã_\u00f1\u00d1\u00E0-\u00FC])\w+/",'message'=>'Sólo se aceptan letras'],
    ...
    public function validateRut($attribute)
    {
        ...
        $recorreString=0;
        $multiplo=2;
        for($i=0;$i<=$count;$i++)//inicio mi ciclo hasta la posiciï¿½n 7
        {
            $resultadoM=$numeros[$recorreString]*$multiplo;//recorro String y multiplico
            $suma=$suma+$resultadoM;//se suma resultado de multiplicaciï¿½n por ciclo
            if($multiplo==7)
            {
                $multiplo=1;
            }
